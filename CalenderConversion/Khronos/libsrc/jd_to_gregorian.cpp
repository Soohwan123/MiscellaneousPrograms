	...
	namespace detail {
		...
		*/
		void jd_to_gregorian_real(jd_t jd, year_t& year, month_t& month, day_t& day) {
			...
			yearday = wjd - gregorian_to_jd(year, 1, 1, 0, 0, 0);
		...
		void jd_to_gregorian_real(jd_t jd, year_t& year, month_t& month, day_t& day, hour_t& hour, minute_t& minute, second_t& second) {
			assert(jd >= -31738.5 && "Minimum convertible date not provided.");
			using namespace utility;
